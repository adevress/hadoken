...
inline void options_handler::add_option(option opt) { _opts.emplace_back(std::move(opt)); }

inline void options_handler::add_subcommand(sub_command sub_com) { _subcmd.emplace_back(std::move(sub_com)); }
...
const std::vector<sub_command>& options_handler::sub_commands() const { return _subcmd; }

const std::vector<option>& options_handler::options() const { return _opts; }

inline string_view options_handler::name() const { return _name; }

inline string_view options_handler::help_message() const { return _help_msg; }
...
std::string options_handler::help(const help_format& fmt) const {
    ...
    std::transform(_subcmd.begin(), _subcmd.end(), std::back_inserter(commands_string),
                   [](const sub_command& s) { return s.name(); });

    std::size_t colmun_length =
        std::max<std::size_t>(_get_max_print_table_column_length(8, 2, options_strings.begin(), options_strings.end()),
...
inline option::option(std::string option_name, std::function<void(const std::string&)> callback, std::string help_msg)
    : _names({std::move(option_name)}), _help_msg(std::move(help_msg)),
      _action([callback](string_view arg) -> void { callback(to_string(arg)); }) {
...
inline string_view option::name() const { return _names.at(0); }
...
inline bool option::_get_flag(int v) const { return _flags[v]; }

inline string_view option::help_message() const { return _help_msg; }
...
bool _is_prefixed_option(string_view candidate_opt) { return (candidate_opt.size() > 0 && candidate_opt[0] == '-'); }
...
inline bool _validate_and_call(std::vector<options_handler const*> stack, string_view prog_name, Iterator begin_arg,
                               Iterator end_arg, bool call, bool positional_only) {
