...
BOOST_AUTO_TEST_CASE(opt_parser) {
    ...
    option opt("--my_opt", [&](std::string v) -> void { my_opt_value = v; }, super_cool_help_message);
    options_h.add_option(opt);
    BOOST_CHECK_EQUAL(opt.help_message(), super_cool_help_message);

    options_h.add_option(
        option("never_called", [&](std::string) -> void { BOOST_FAIL("should not reach here"); }, "not so useful message"));


    options_h.set_positional_argument_handler([&](std::string a) { positionals.emplace_back(a); });
...
BOOST_AUTO_TEST_CASE(opt_parser_subcommand) {
    ...
    options.add_option(option("--little-opt", [&](std::string v) { BOOST_CHECK_EQUAL(v, my_argument_value); }, "help dude"));


    options.add_option(option("--nothing-interesting",
                              [&](std::string v) { BOOST_FAIL(hadoken::scat("option should not be called", v)); },
                              "help dude"));
    ...
    sub_command sub_comm("sub_exec", [&]() { BOOST_TEST_CHECKPOINT("Sub exec scope triggered"); }, "help msg");

    sub_comm.add_option(option("--subarg",
                               [&](std::string v) {
    ...
    sub_comm.set_positional_argument_handler([&](std::string v) { positionals_sub_not_called.emplace_back(v); });

    options.add_subcommand(sub_comm);

    options.add_subcommand(sub_command("other_command", [&]() { BOOST_FAIL("should not be called"); }, "help msg"));



    std::vector<string_view> args = {"--little-opt", my_argument_value, "sub_exec", "--subarg", my_argument_value2, "pos1",
                                     "pos2"};
...
BOOST_AUTO_TEST_CASE(opt_parser_throw_end_of_list) {
    ...
    options.add_option(option("--opt", [](std::string v) { BOOST_FAIL(hadoken::scat("should not reach here", v)); }, "msg"));
...
BOOST_AUTO_TEST_CASE(opt_parser_type_check) {
    ...
    options.add_option(option("--opt-str", [&](std::string v) { BOOST_CHECK_EQUAL(v, str_value); }, "msg"));

    options.add_option(option("--opt-int", [&](int v) { BOOST_CHECK_EQUAL(v, int_value); }, "msg"));

    options.add_option(option("--opt-nothing", [&]() { BOOST_TEST_CHECKPOINT("should be called properly"); }, "msg"));
...
BOOST_AUTO_TEST_CASE(opt_parser_alias) {
    ...
    option opt_nothing("--opt-nothing", [&]() { counter += 1; }, "msg");
