...
typedef boost::mpl::list<short int, short unsigned int, int, unsigned int, long, unsigned long, long long, unsigned long long,
                         double, float, long double, std::string>
    small_vector_types;
...
BOOST_AUTO_TEST_CASE_TEMPLATE(concurrent_queue_test_pthread, T, small_vector_types) {
    ...
    for (std::size_t i = 0; i < nb_input_items; ++i) {
        threads_pool_push.emplace_back(std::thread([&items, &queue, i]() { queue.push(items[i]); }));
    }
